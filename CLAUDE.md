...
- **camelCase on the wire.** Every JSON property (request bodies, response shapes, query parameters) is camelCase. Backend uses Pydantic v2 `alias_generator=to_camel + populate_by_name=True + serialize_by_alias=True` to derive aliases from snake_case Python attribute names. SQL columns stay snake_case. See [ADR 0009](./docs/adrs/0009-camelcase-on-the-wire.md).
